...
function ResultSection({ icon: Icon, title, color, items, renderItem }) {
  ...
  return (
    <section className="mb-8">
      ...
      {items.length > 10 && (
        <p className="text-xs text-gray-400 mt-2 text-right">僅顯示前 10 筆，請縮小關鍵字範圍</p>
      )}
    </section>
  )
}
      ...
      <div className="mb-8">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-900 shadow-sm bg-white"
            autoFocus
          />
        </div>
        {q && !loading && (
      ...
      {loading ? (
        ...
        <>
          ...
          <ResultSection
            ...
            renderItem={s => (
              <Link key={s.id} to={`/singles/${s.id}`}
                ...
                <div className="w-9 h-9 rounded bg-blue-100 overflow-hidden shrink-0">
                  {s.imageUrl
                    ? <img src={s.imageUrl} alt={s.title} className="w-full h-full object-cover" />
                    : <div className="w-full h-full flex items-center justify-center"><Music size={14} className="text-blue-500" /></div>
                  }
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-gray-900 truncate">{s.title}</div>
                  <div className="text-xs text-gray-400">{s.artistName} · {s.year ? `${s.year}年` : ''}</div>
                </div>
            ...
            renderItem={a => (
              <Link key={a.id} to={`/albums/${a.id}`}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-400">{a.artistName} · {a.year ? `${a.year}年` : ''}</div>
                </div>
                <ArrowRight size={14} className="text-gray-300 shrink-0" />
              </Link>
            )}
              ...
              <Link key={v.id} to={`/video-works/${v.id}`}
                ...
                <div className="w-9 h-9 rounded bg-gray-100 overflow-hidden shrink-0">
                  {v.imageUrl
                    ...
                    : <div className="w-full h-full flex items-center justify-center"><Video size={14} className="text-gray-500" /></div>
                  }
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-gray-900 truncate">{v.title}</div>
                  <div className="text-xs text-gray-400">{v.artistName} · {v.year ? `${v.year}年` : ''}</div>
                </div>
                <ArrowRight size={14} className="text-gray-300 shrink-0" />
              </Link>
            )}
          ...
          <ResultSection
            ...
            renderItem={(s, i) => (
              <div key={s.id || i} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50">
                <div className="w-9 h-9 rounded bg-rose-50 overflow-hidden shrink-0">
                  {s.imageUrl
                    ? <img src={s.imageUrl} alt={s.title} className="w-full h-full object-cover" />
