...
const KIND_COLOR = {
  single: 'bg-blue-900/30 text-blue-400',
  digital: 'bg-green-900/30 text-green-700',
  album: 'bg-indigo-100 text-indigo-400',
  coupling: 'bg-zinc-800 text-zinc-400',
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-1">
          <Mic2 size={22} className="text-rose-400" />
          <h1 className="text-2xl font-bold text-white">提供樂曲</h1>
        </div>
        <p className="text-sm text-zinc-500">小室哲哉作曲・編曲，但非本人監製的樂曲</p>
      </div>
                ...
                <button
                  ...
                  className={`flex items-center justify-between px-3 py-2.5 rounded-xl border text-left transition-all gap-2 ${
                    isSelected
                      ...
                      : 'bg-zinc-900 border-white/10 hover:border-rose-300 hover:bg-rose-900/20 text-zinc-100'
                  }`}
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className={`w-8 h-8 rounded-full overflow-hidden shrink-0 ${isSelected ? 'ring-2 ring-white/50' : 'ring-1 ring-gray-200'}`}>
                      {imgUrl
                        ...
                        : <div className={`w-full h-full flex items-center justify-center text-xs font-bold ${isSelected ? 'bg-rose-900/200 text-white' : 'bg-rose-100 text-rose-400'}`}>
                            {name[0]?.toUpperCase() || '?'}
                          </div>
                      }
                    </div>
                  ...
                  <span className={`text-xs font-bold shrink-0 px-1.5 py-0.5 rounded-full ${
                    isSelected ? 'bg-rose-900/200 text-white' : 'bg-rose-100 text-rose-400'
                  }`}>
                    {count}
                  </span>
                </button>
      ...
      {filterArtist && !loading && (
        <div className="mb-6 p-4 bg-rose-900/20 border border-rose-200 rounded-xl">
          <div className="flex items-center justify-between mb-1">
            ...
            <button
              ...
              className="p-1 rounded-lg hover:bg-rose-100 text-rose-400 hover:text-rose-400 transition-colors"
            >
              <X size={16} />
            </button>
          </div>
      ...
      {loading ? <LoadingSpinner /> : (
        <div className="space-y-4">
          {groupedFiltered.length === 0 ? (
            <p className="text-center py-12 text-zinc-500 text-sm bg-zinc-900 rounded-xl border border-white/10">
              沒有符合條件的結果
            </p>
          ) : (
            groupedFiltered.map((group, gi) => (
              <div key={gi} className="bg-zinc-900 rounded-xl border border-white/10 shadow-sm overflow-hidden">
                ...
                {group.sourceTitle ? (
                  ...
                  <div className="px-4 py-2 bg-zinc-900/50 border-b border-white/8">
                    <span className="text-xs text-zinc-500">未分類</span>
                  </div>
                )}
                  ...
                  {group.songs.map((song, si) => (
                    <div key={song.id || si} className="px-4 py-3 flex items-start gap-3 hover:bg-zinc-900/50">
                      ...
                      {!group.imageUrl && song.imageUrl && (
                        <div className="w-8 h-8 rounded overflow-hidden bg-rose-900/20 shrink-0 mt-0.5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {!filterArtist && (
                          <button
                            ...
                            className="text-xs text-rose-400 hover:text-rose-800 font-medium mb-1 flex items-center gap-0.5 hover:underline"
                          >
                            {song.artistName}
                      ...
                      <div className="text-xs text-zinc-500 shrink-0 pt-0.5 whitespace-nowrap">
